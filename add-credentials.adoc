...
Obtén más información sobre los permisos que se requieren para cada capacidad de carga de trabajo en la link:permissions-reference.html["Referencia sobre permisos"].
...
Tu conjunto inicial de credenciales de AWS puede incluir una política de permisos de IAM para una o varias funcionalidades.  Depende de las necesidades de tu negocio.
...
. En la página Add Credentials, selecciona *Add manually* y luego usa los siguientes pasos para completar cada sección en _Permissions configuration_.
+
...
. Para aquellas funcionalidades de carga de trabajo que ofrecen una selección de políticas de permisos, seleccione el tipo de permisos que estarán disponibles con estas credenciales.
+

NOTE: Si tienes una cuenta de GovCloud, no necesitas los permisos de Cost Explorer `ce:GetCostAndUsage` o `ce:GetTags` en la política **Visualización, planificación y análisis** de Almacenamiento. Después de copiar el script del cuadro de código, elimina estos permisos y borra la coma final de la línea anterior.
...
. Copie el rol ARN.
. Vuelve a la página Add Credentials en Workload Factory y luego pega el role ARN en el campo _Role ARN_.
...
NOTE: No se admite la adición de credenciales a una cuenta de AWS GovCloud (US) mediante CloudFormation.

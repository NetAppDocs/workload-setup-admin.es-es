...
Conectividad de Lambda link:: Utiliza la conectividad de Lambda link para crear una relación de confianza y conectividad entre una cuenta de Workload Factory y uno o varios sistemas de archivos FSx for ONTAP. Los enlaces te permiten supervisar y gestionar ciertas funciones del sistema de archivos directamente desde las llamadas a la API de REST de ONTAP que no están disponibles a través de la API de Amazon FSx for ONTAP.
...
Operaciones de almacenamiento de EDA:: Optimiza tus operaciones de almacenamiento de automatización del diseño electrónico (EDA) en NetApp Workload Factory optimizando el rendimiento, integrando con tu IaC para automatizar los parámetros de almacenamiento y asegurando la salud operativa.
...
NOTE: Los administradores de la cuenta pueden activar o desactivar las funciones de IA. Si un administrador las desactiva, el asistente de IA Ask Me no estará disponible. link:manage-genai-features.html["Aprende sobre cómo gestionar las funciones de IA"].
...
link:https://docs.netapp.com/us-en/workload-fsx-ontap/links-overview.html["Más información sobre los enlaces"^]

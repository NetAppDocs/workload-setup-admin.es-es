...
---
= Gestiona cuentas de servicio en NetApp Workload Factory
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Opcionalmente, puedes obtener un token de acceso para la API de gestión Auth0 ejecutando un intercambio de credenciales de cliente. Consulta link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["Documentación de la API de NetApp Console"^] para información sobre cómo generar un token de actualización.
+
El ejemplo de curl muestra cómo puedes tomar el client ID y el secret y usar una API para generar el token de acceso, que tiene un tiempo limitado. El token proporciona varias horas de acceso a las APIs de NetApp Workload Factory.
...
----
+
Publica la salida en https://netapp-cloud-account.auth0.com/oauth/token para obtener el token de acceso.
